...
__global__ void gemm_kernel(void* Cptr, const void* Aptr, const void* Bptr, int m, int n, int k) {
    ...
    auto copy_safe = [&](auto S, auto D, auto C, int maxMN, bool clearD = true){
    ...
    for(int i = 0; i < k_stage - 1; ++i) {
        copy_safe(tAgA(_,_,_,i), tAsA(_,_,_,i), tAcA, maxM);
        copy_safe(tBgB(_,_,_,i), tBsB(_,_,_,i), tBcB, maxN);
        cp_async_fence();
    }
    ...
    for(int itile = 0; itile < num_tile_k; ++itile) {
        ...
        for(int ik = 0; ik < nk; ++ik) {
            ...
            if (ik == 0 && itile < num_tile_k - (k_stage - 1)) {
                ...
                copy_safe(tAgA(_,_,_,load_idx), tAsA(_,_,_,smem_write_stage), tAcA, maxM);
                copy_safe(tBgB(_,_,_,load_idx), tBsB(_,_,_,smem_write_stage), tBcB, maxN);
