...
void run_hstu_fwd_impl(Hstu_fwd_params& params, cudaStream_t stream) {
  ...
  if constexpr(!Is_mfalcon)
  {
    using True_t = fwdMask_kernel_traits<
        kHeadDim, kBlockM, kBlockN, kNWarps, Has_rab, Is_even_rab,
        -1,
        elem_type>;
    using False_t = Hstu_fwd_kernel_traits<
    ...
    using Kernel_traits = typename std::conditional<Has_mask, True_t, False_t>::type;
    ...
    auto kernel = [&] {
      if constexpr (Has_mask) {
          return &flash::hstu_fwd_kernel_mask<Kernel_traits, Hstu_fwd_params>;
      } else {
          return &flash::hstu_fwd_kernel<Kernel_traits, Hstu_fwd_params>;
      }
    }();
  ...
  {
     // for mfalcon
    using Kernel_traits = Mfalcon_fwd_kernel_traits<kHeadDim,kBlockM,kBlockN,kNWarps,Has_rab,Is_even_rab,Has_mask,elem_type>;
    constexpr size_t smem_size = Kernel_traits::kSmemSize;
    const int num_m_block = (params.seqlen_q + kBlockM - 1) / kBlockM; 
    // params.num_blocks_per_batch = num_m_block * params.h;
    // params.step = 0;
    dim3 grid;
    if constexpr (Is_balance) {
      grid = dim3(params.h, params.b, num_m_block);
    ...
    auto kernel = &flash::hstu_mfalcon_persistent<Kernel_traits, Hstu_fwd_params>;
    if constexpr (smem_size >= 48 * 1024) {
      C10_CUDA_CHECK(cudaFuncSetAttribute(
          kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
    }
    ...
    // void* args[] = {&params};
    // cudaLaunchCooperativeKernel((const void*)kernel, grid, Kernel_traits::kNThreads, args, smem_size, stream);
    kernel<<<grid, Kernel_traits::kNThreads, smem_size, stream>>>(params);
    C10_CUDA_KERNEL_LAUNCH_CHECK();
...
template <int Arch, typename elem_type, int kHeadDim, bool Has_rab, bool Is_local,
          bool Is_causal, bool Is_context, bool Is_target, bool Is_delta_q,bool Is_mfalcon,bool Has_mask>
void run_hstu_fwd_(Hstu_fwd_params& params, cudaStream_t stream) {
  ...
  INT_SWITCH(params.page_size, Page_Size, [&] {
    ...
    static constexpr auto tile_size = flash::get_tile_size_fwd<Arch, kHeadDim, Has_rab,Is_mfalcon,Has_mask>();
    ...
    BOOL_SWITCH(even_rab, Is_even_rab, [&] {
      run_hstu_fwd_impl<elem_type, kHeadDim, kBlockM, kBlockN, kNWarps, Is_delta_q, Is_causal, Is_target, Is_context, Is_local,
                        Has_rab, Paged_KV, Is_balance, Is_even_rab, Is_Q_in_regs, Share_Q_K_smem,Is_mfalcon,Has_mask>(params, stream);
    });
  });
  // });
}