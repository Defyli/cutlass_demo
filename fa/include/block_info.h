...
template <typename Kernel_traits, typename Params>
struct MfalconBlockInfo
{
  __device__ MfalconBlockInfo(const Params &params,const int bidb):
  sum_s_q(params.cu_seqlens_q[bidb]),
  sum_s_k(params.cu_seqlens_k[bidb]),
  sum_mfalcon_cache(params.cu_seqlens_cache[bidb]),  // u+micro_bs | u+micro_bs | ...
  q_step_offset(params.step==0?0:params.user_length[bidb]+params.step*params.micro_bs[bidb]),
  actual_seqlen_q(params.step == 0
                  ? MIN(params.cu_seqlens_q[bidb + 1] - sum_s_q, params.user_length[bidb] + params.micro_bs[bidb])
                  : MIN(params.micro_bs[bidb], (params.cu_seqlens_q[bidb + 1] - sum_s_q) - q_step_offset)),
  actual_seqlen_k(MIN(params.cu_seqlens_k[bidb + 1] - sum_s_k,params.user_length[bidb]+params.micro_bs[bidb])),
  actual_seqlen_t(params.micro_bs[bidb]){}

  template<typename index_t>
  __forceinline__ __device__ index_t mfalcon_cache_offset(const index_t row_stride)const{
      return uint32_t(sum_mfalcon_cache)*row_stride;  //assum k.shape==v.shape
  }

  template <typename index_t>
  __forceinline__ __device__ index_t q_offset(const index_t row_stride) const {
    return uint32_t(sum_s_q+q_step_offset) * row_stride;
  }
  ...
  const int sum_mfalcon_cache;
  const int q_step_offset;
