...
void set_params_fprop(Hstu_fwd_params* params,
                      ...
                      // mfalcon参数         
                      bool is_mfalcon=false,
                      void*user_length=nullptr,
                      void*micro_bs=nullptr,
                      void*target_length=nullptr) {
  ...
  *params = {};
  ...
  if (has_rab) {
    ...
    params->rab_seqlen_qk_stride = rab.stride(-4); // (batch_size, nhead_rab, max_seqlen_q, max_seqlen_k), relative attention bias
  ...
  params->has_mask = attn_mask.defined() && attn_mask.numel()>0;
  if(params->has_mask)
  {
    params->mask_ptr = attn_mask.data_ptr();
    params->mask_batch_stride = attn_mask.stride(0);
    params->mask_row_stride = attn_mask.stride(1);
  }   
  ...
  if(is_mfalcon)
  {
      ...
      TORCH_CHECK(!params->is_mfalcon, "This hstu attention build does not support mfolcon.")
      TORCH_CHECK(!params->is_target,"Mfalcon must be compiled with target off"); 
      TORCH_CHECK(!params->is_paged_kv,"Mfalcon must be compiled with paged_kv off");
      TORCH_CHECK(!params->is_local,"Mfalcon must be compiled with local off");
      TORCH_CHECK(!params->is_delta_q,"Mfalcon must be compiled with delta_q off");
      TORCH_CHECK(!params->is_causal,"Mfalcon must be compiled with causal mask off");
      TORCH_CHECK(k.size(1)==v.size(1)&&k.size(2)==v.size(2),"Mfalcon only support k and v have the same shape for this version.");
    #endif
    params->is_mfalcon = is_mfalcon;
    params->user_length = reinterpret_cast<int*>(user_length);
    params->micro_bs = reinterpret_cast<int*>(micro_bs);
    params->target_length = reinterpret_cast<int*>(target_length);   

    #ifdef MFALCON_DEBUG
      unsigned int*user_length_host = new unsigned int[b];
      unsigned int*micro_bs_host = new unsigned int[b];
      cudaMemcpy(user_length_host, user_length, b * sizeof(unsigned int), cudaMemcpyDeviceToHost);
      cudaMemcpy(micro_bs_host, micro_bs, b * sizeof(unsigned int), cudaMemcpyDeviceToHost);
      printf("batch size is %d\n",b);
      printf("user_length %u\n",user_length_host[0]);
      printf("micro_bs %u\n",micro_bs_host[0]);
...
template <typename Dtype, bool Has_rab, bool Is_local,
          bool Is_causal, bool Is_context, bool Is_target, bool Is_delta_q,bool Is_mfalcon = false, bool Has_mask = false>
void run_hstu_fwd_headdim(Hstu_fwd_params &params, cudaStream_t stream) {
  ARCH_SWITCH(params.arch, Arch, [&] {
    #ifndef HSTU_DISABLE_HDIM32
    if (params.d == 32) { run_hstu_fwd_<Arch, Dtype, 32, Has_rab, Is_local, Is_causal, Is_context, Is_target, Is_delta_q,Is_mfalcon,Has_mask>(params, stream); }
    #endif
    #ifndef HSTU_DISABLE_HDIM64
    if (params.d == 64) { run_hstu_fwd_<Arch, Dtype, 64, Has_rab, Is_local, Is_causal, Is_context, Is_target, Is_delta_q,Is_mfalcon,Has_mask>(params, stream); }
    #endif
    #ifndef HSTU_DISABLE_HDIM128
    if (params.d == 128) { run_hstu_fwd_<Arch, Dtype, 128, Has_rab, Is_local, Is_causal, Is_context, Is_target, Is_delta_q,Is_mfalcon,Has_mask>(params, stream); }
    #endif
    #ifndef HSTU_DISABLE_HDIM256
    if (params.d == 256) { run_hstu_fwd_<Arch, Dtype, 256, Has_rab, Is_local, Is_causal, Is_context, Is_target, Is_delta_q,Is_mfalcon,Has_mask>(params, stream); }
    #endif
  });
}
...
void run_hstu_fwd(Hstu_fwd_params &params, cudaStream_t stream) {
    RAB_SWITCH(params.has_rab, Has_rab, [&] {
      FP16_BF16_SWITCH(params.is_bf16, [&] {
        MASK_SWITCH(params.has_mask, Has_mask, [&] {
          #ifndef HSTU_MFALCON_DISABLE
            if(params.is_mfalcon){
                // M-Falcon path is mutually exclusive with other modes.
                // Dispatch directly to the M-Falcon kernel implementation.
                // The other flags (Is_local, Is_causal, etc.) are set to false
                // because M-Falcon's kernel handles its own specific logic.

                    run_hstu_fwd_headdim<Dtype, Has_rab,
                                        /*Is_local=*/false,
                                        /*Is_causal=*/false,
                                        /*Is_context=*/false,
                                        /*Is_target=*/false,
            ...
            if(Has_mask){
              //Mask path is mutually exclusive with other modes.
              run_hstu_fwd_headdim<Dtype, Has_rab,
                                        /*Is_local=*/false,
                                        /*Is_causal=*/false,
                                        /*Is_context=*/false,
        ...
        if (params.is_delta_q) {
          ...
          if (params.is_local) { run_hstu_fwd_headdim<Dtype, Has_rab, true, false, false, false, true,false>(params, stream); return; }
          #endif
          #ifndef HSTU_DISABLE_TARGET
          if (params.is_target) { run_hstu_fwd_headdim<Dtype, Has_rab, false, true, false, true, true,false>(params, stream); return; }
          #endif
          run_hstu_fwd_headdim<Dtype, Has_rab, false, true, false, false, true,false>(params, stream); return;
        }
        #endif
        #ifndef HSTU_DISABLE_LOCAL
        if (params.is_local) { run_hstu_fwd_headdim<Dtype, Has_rab, true, false, false, false, false,false>(params, stream); return; }
        #endif
        if (!params.is_causal) { run_hstu_fwd_headdim<Dtype, Has_rab, false, false, false, false, false,false>(params, stream); return; }
        else {
          #ifndef HSTU_DISABLE_CAUSAL
          CONTEXT_SWITCH(params.is_context, Is_context, [&] {
            TARGET_SWITCH(params.is_target, Is_target, [&] {
              run_hstu_fwd_headdim<Dtype, Has_rab, false, true, Is_context, Is_target, false,false>(params, stream);
            });
          });
          #endif
        }
        TORCH_CHECK(false, "No instances are vailed! check compile args!");
      });
    });
  });
}
...
std::vector<at::Tensor> hstu_varlen_fwd(
  ...
  if (has_mask) {
    CHECK_DEVICE(attn_mask.value());
    TORCH_CHECK(attn_mask.value().stride(-1) == 1, "attn_mask must have contiguous last dimension");
    CHECK_SHAPE(attn_mask.value(), batch_size, max_seqlen_q, max_seqlen_k);
    if (seqlen_k_rounded != max_seqlen_k) {
        attn_mask = torch::nn::functional::pad(attn_mask.value(), torch::nn::functional::PadFuncOptions(
                            {0, seqlen_k_rounded - max_seqlen_k}));
    }
    TORCH_CHECK(!is_delta_q && !is_paged_kv && window_size_left == -1 && window_size_right == -1,
                "Attn_mask is not compatible with is_delta_q, is_paged_kv, or window_size.");
  }
  if (is_mfalcon.has_value() && is_mfalcon.value()) {
    TORCH_CHECK(user_length.has_value(), "M-Falcon mode requires user_length tensor.");
    TORCH_CHECK(micro_bs.has_value(), "M-Falcon mode requires micro_bs tensor.");

    int user_len_val = user_length.value().cpu().index({0}).item<int>();
    int micro_bs_val = micro_bs.value().cpu().index({0}).item<int>();

    // m-falcon will cut the tensor into user part and target part
    // both part statisfy alignment requirement (alignment to 16bytes)
    const int alignment = sizeof(cutlass::uint128_t) / q.element_size();
    TORCH_CHECK(user_len_val % alignment == 0, "For M-Falcon mode, user_length must be a multiple of ", alignment, " for memory alignment reasons. Please pad your inputs accordingly.");
    TORCH_CHECK(micro_bs_val % alignment == 0, "For M-Falcon mode, micro_bs must be a multiple of ", alignment, " for memory alignment reasons. Please pad your inputs accordingly.");
  }
  ...
  set_params_fprop(&params,                  //
                   ...
                   window_size_left,         //
                   window_size_right,
                    // Pass M-Falcon parameters' pointers
                   attn_mask.has_value() ? attn_mask.value() : at::Tensor(),
                   is_mfalcon.has_value()?is_mfalcon.value():false,
                   user_length.has_value() ? user_length.value().data_ptr(): nullptr,
                   micro_bs.has_value() ? micro_bs.value().data_ptr() : nullptr,
                   target_length.has_value() ? target_length.value().data_ptr() : nullptr);       //
  if (max_seqlen_k > 0) {
...
#ifndef PYBIND11_DISABLE
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.doc() = "HstuAttention";
    m.def("hstu_attn_varlen_func", &hstu_varlen_fwd, "Varlen hstu forward pass",
        py::arg("q"),
        py::arg("k"),
        py::arg("v"),
        py::arg("cu_seqlens_q"),
        ...
        py::arg("page_offsets"),
        py::arg("page_ids"),
        py::arg("last_page_lens"),
        py::arg("cu_seqlens_t"),
        // Add M-Falcon args to pybind definition
        py::arg("attn_mask") = std::nullopt,
        py::arg("is_mfalcon") = std::optional<const bool>{false},
        py::arg("user_length")= std::nullopt,
        py::arg("micro_bs") = std::nullopt,
        py::arg("target_length") = std::nullopt
  );
