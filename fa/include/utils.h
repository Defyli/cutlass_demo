...
template <typename TiledCopy,
          ...
          typename Engine2,
          typename Layout2>
__forceinline__ __device__ void copy_with_dual_bound_check(
    TiledCopy tiled_copy,
    Tensor<Engine0, Layout0> const& S,
    Tensor<Engine1, Layout1>& D,
    Tensor<Engine2, Layout2> const& identity_MN,
  ...
  for (int m = 0; m < size<1>(S); ++m) {
    if (get<0>(identity_MN(0, m, 0)) < max_M) {
      #pragma unroll
      for (int k = 0; k < size<2>(S); ++k) {
        if (get<1>(identity_MN(0, m, k)) < max_N) {
            cute::copy(tiled_copy, S(_, m, k), D(_, m, k));
        }
      }
    }
...
template <int Arch, int kHeadDim, bool Has_rab,bool Is_mfalcon,bool Has_mask>
constexpr std::tuple<int, int, int> get_tile_size_fwd() {
  if constexpr(Is_mfalcon||Has_mask)
  {
      return {128,64,8};
  }
